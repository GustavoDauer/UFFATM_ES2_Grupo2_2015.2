 ...
 * @author Igor
 */
public class TransferenciaTest extends Mockito {
    ...
    public void setUp() {
        ...
        cliente = new Cliente();
        cliente.setId(String.valueOf(id + 2));
        cliente.setNome("Teste");
        cliente.setStatus("1");
        cliente.insert();
        ...
        conta.setAgencia("0");
        conta.setBanco("0");
        conta.setId(String.valueOf(id + 2));
        conta.setIdCliente(String.valueOf(id + 2));
        conta.setLimite("1000");
        conta.setListaClientes(listCliente);
        conta.setNumeroCartao(String.valueOf(id + 2));
        conta.setStatus("1");
        conta.setPoupanca("0");
        conta.setPoupanca_centavos("0");
        conta.setPoupanca_status("1");
    ...
    @Test
    public void transferenciaSaqueInsuficiente() {
        when(request.getParameter("valor")).thenReturn("5000");
        when(request.getParameter("idContaTransferencia")).thenReturn(String.valueOf(id + 2));
        transferencia = new Transferencia(request);

        assertEquals(false, transferencia.insert());
    }

    @Test
    public void transferenciaSaqueSuficiente() {
        when(request.getParameter("valor")).thenReturn("100");
        when(request.getParameter("idContaTransferencia")).thenReturn(String.valueOf(id + 2));
        transferencia = new Transferencia(request);

        assertEquals(true, transferencia.insert());
    }
